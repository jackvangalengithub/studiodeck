...
**Review before sharing.** Each import processes up to 120 pages, 40 image regions per page, and 60 MB of extracted images. Limits and individual page failures are reported. Each page retains its own text; the combined AI financial analysis uses up to 100 KB of source text and reports truncation. Image regions follow their visible placement, including cropping and page rotation. Flattened moodboards can be separated at clear white gutters. A dense-image boundary pass trims paper margins and sparse caption bands, including single-photo scans and monochrome photos. Complex collages, overlapping text and very pale artwork can still need review; page previews retain the complete source layout. Use **Extract again** in a draft to apply improved crops to an older upload. OCR can miss or misread text. LibreOffice font availability can affect slide rendering; unsupported embedded image formats are reported in the fallback. Saved spreadsheet formula caches can be stale. AI visual descriptions and style are suggestions, and source prices and tax treatment still require review.
