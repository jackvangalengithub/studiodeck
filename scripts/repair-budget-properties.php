...
$changes=transaction(function()use($pid,$options){
    $changes=[];foreach(rows("SELECT b.* FROM budget_items b JOIN iterations i ON i.id=b.iteration_id WHERE i.project_id=? AND i.status='draft' AND b.source_version_id IS NOT NULL",[$pid]) as $item){
