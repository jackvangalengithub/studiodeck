// Opt-in acceptance check for the installed, local Villa Auren demo.
...
const database=process.env.VILLA_DEMO_DATABASE,base=process.env.VILLA_DEMO_URL;
if(!database||!base)throw Error('Set VILLA_DEMO_DATABASE and VILLA_DEMO_URL to the local demo instance.');
const studio='85c6b0cceb613adb3b2c8e0db4f63d25',pid='232de98e5f865db9782c6e29f5534726',iid='09c9fd46c575900d71552da70845cf58';
...
(async()=>{
  ...
  try{
    ...
    assert.equal(data.files.length,32);assert.equal(data.budget.length,18);
    const {budgetTotal}=await import(path.resolve(__dirname,'../public/assets/budget.js'));
    ...
    assert.equal(budgetTotal(baseline),27850000);
    const all=data.budget.map(r=>({...r,selected:true,range_percent:100}));
    assert.equal(budgetTotal(all),31460000);
    assert.equal(budgetTotal(baseline.map(r=>({...r,range_percent:100}))),28650000);
    const controls=data.budget.find(r=>r.label==='Dimming drivers & scene controls');
    ...
    const dir=path.resolve(__dirname,'../demos/villa-auren/checks');fs.mkdirSync(dir,{recursive:true});
    ...
    const slides=presentationSlides(data);assert.ok(slides.length>=29);
    const additions=JSON.parse(fs.readFileSync(path.resolve(__dirname,'../demos/villa-auren/additional-slides.json')));
    ...
    for(const slide of slides){
      ...
      if(slide.type==='fullphoto'&&slide.title==='The art of staying in.')await page.screenshot({path:path.join(dir,'living-room.png')});
      if(slide.id==='budget'){
    ...
    for(const f of data.files.filter(f=>/FB-260918|LW-260918|SA-260918|scope/.test(f.name))){
    ...
    const report={nativeSlides:slides.length,files:data.files.length,budgetRows:data.budget.length,base:278500,premium:286500,allOptions:314600,allImagesDecoded:true,nestedSupplierDownloads:true,browserErrors:errors};
