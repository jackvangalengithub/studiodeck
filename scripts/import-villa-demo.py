"""Add Villa Auren to an existing StudioDeck studio without replacing any project.
...
def uid(key):return hashlib.sha256(('villa-auren-demo-v1:'+a.studio+':'+key).encode()).hexdigest()[:32]
...
required=['Villa-Auren-Presentation.pdf','scope.pdf','budget.csv','materials.csv','programme.csv']
...
backup=backup_dir/('before-villa-auren-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S')+'.sqlite')
...
def add_file(rel,category,name=None):
    ...
    meta=dict(demo=True,review_required=False,summary='Fictional Villa Auren demonstration source.',generated=path.suffix=='.webp')
    prv=None;text='';pages=[]
...
try:
    ...
    ins('projects',id=pid,user_id=a.owner,studio_id=a.studio,name='Villa Auren · Modern villa demo',location=manifest['location'],description='A quieter kind of luxury. A complete fictional modern-villa interior concept with original AI imagery, material boards and a transparent supplier budget.',theme=theme,created_at=stamp,visibility='team',archived=0)
    ins('project_members',project_id=pid,user_id=a.owner)
    ins('project_pins',project_id=pid,user_id=a.owner)
    ins('project_details',project_id=pid,tags=json.dumps(['Demo 01','Modern villa','Fictional project']),deadline='')
    ins('iterations',id=iid,project_id=pid,number=1,title='Villa Auren · Complete interior concept',status='draft',theme=theme,created_at=stamp)
    for n,(gid,label) in enumerate([('story','The story'),('current','The architecture'),('moodboards','Materials & mood'),('designs','The designs'),('budget','The investment'),('delivery','The next chapter')]):
        ins('slide_groups',iteration_id=iid,id=gid,label=label,position=n)
    for key in manifest['assets']:
        add_file('assets/'+key+'.webp','moodboard' if key in ['materials','softboard'] else 'drawings' if key.startswith('sketch-') else 'renders',name='Villa Auren — '+key+' — AI concept.webp')
    deckvid=add_file('Villa-Auren-Presentation.pdf','other')
    add_file('scope.pdf','legal','Villa Auren — scope & assumptions.pdf')
    add_file('budget.csv','budget','Villa Auren — master budget.csv')
    add_file('materials.csv','other','Villa Auren — material specification.csv')
    add_file('programme.csv','other','Villa Auren — delivery programme.csv')
    ...
    # Room imagery, moodboards and text remain native editable slide records.
    composed={'palette','hierarchy','options','programme'}
    for n,s in enumerate(manifest['slides']):
        ...
        elif s['image']=='floorplan':
            # Vector source is rendered to PNG for StudioDeck's supported image types.
            svg=(bundle/'assets/floorplan.svg').read_bytes()
            converted=fitz.open('svg',svg).convert_to_pdf();doc=fitz.open('pdf',converted)
            plan=doc[0].get_pixmap(matrix=fitz.Matrix(2,2),alpha=False).tobytes('png')
            (bundle/'assets/floorplan.png').write_bytes(plan)
            vid=add_file('assets/floorplan.png','drawings','Villa Auren — schematic floor plan.png');typ='floorplan'
        elif s['image']:
            vid=versions['assets/'+s['image']+'.webp']
            typ='moodboard' if s['section']=='moodboards' else 'fullphoto' if s['kind'] in ['hero','cover'] else 'render'
        typ=s.get('type',typ)
        sid=uid('slide:'+s['key'])
        ins('presentation_slides',id=sid,iteration_id=iid,source_version_id=vid,page_number=page,image_number=0,type=typ,situation='concept' if vid else 'unknown',title=s['title'],description=desc,metadata=json.dumps(dict(confidence='manual',demo=True,evidence='Original fictional demonstration content; AI-generated concept imagery.',palette=[{'hex':c} for c in manifest['theme']['colors']])),position=n,manual=1)
    ...
    for sid,title,description,section,position in [
        ('intro','Villa Auren. A quieter kind of luxury.',manifest['slides'][0]['body']+' All clients, suppliers and prices are fictional; imagery is AI-generated.','story',0),
        ('budget','The investment, made clear.','Fictional demo: €278,500–€286,500 including €25,000 contingency. Options begin unselected; landscape work remains unpriced. Assumed VAT included. Expand supplier rows to explore included subquotes.','budget',next(n for n,s in enumerate(manifest['slides']) if s['key']=='investment')),
        ...
        ('summary','Every detail, together.','Download the designed presentation, original images, 15 fictional supplier quotes, material schedule, programme and scope document.','delivery',len(manifest['slides'])+2),
    ]:
