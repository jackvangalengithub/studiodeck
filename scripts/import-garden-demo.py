"""Add Stillwater Garden to an existing StudioDeck studio without replacing any project.
...
def uid(key):return hashlib.sha256(('stillwater-garden-demo-v1:'+a.studio+':'+key).encode()).hexdigest()[:32]
...
required=[manifest['pdf'],'scope.pdf','budget.csv','materials.csv','planting.csv','programme.csv']
...
backup=backup_dir/('before-stillwater-garden-'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S')+'.sqlite')
...
def add_file(rel,category,name=None):
    ...
    meta=dict(demo=True,review_required=False,summary='Fictional Stillwater Garden demonstration source.',generated=path.suffix=='.webp')
    prv=None;text='';pages=[]
...
try:
    ...
    ins('project_details',project_id=pid,tags=json.dumps(['Demo 02','Garden design','Fictional project']),deadline='')
    ins('iterations',id=iid,project_id=pid,number=1,title='Stillwater Garden · Complete landscape concept',status='draft',theme=theme,created_at=stamp)
    ...
    for key in manifest['assets']:
        add_file('assets/'+key+'.webp','moodboard' if key in ['materials','botanical','seasons'] else 'drawings' if key.startswith('sketch-') else 'renders',name='Stillwater Garden — '+key+' — AI concept.webp')
    deckvid=add_file('Stillwater-Garden-Presentation.pdf','other')
    add_file('scope.pdf','legal','Stillwater Garden — scope & assumptions.pdf')
    add_file('budget.csv','budget','Stillwater Garden — master budget.csv')
    add_file('materials.csv','other','Stillwater Garden — material specification.csv')
    add_file('programme.csv','other','Stillwater Garden — delivery programme.csv')
    add_file('planting.csv','other','Stillwater Garden — candidate planting schedule.csv')
    ...
    # Garden imagery, moodboards and text remain native editable slide records.
    ...
    for n,s in enumerate(manifest['slides']):
        ...
        if s['key'] in composed:
            ...
            vid=add_file('assets/'+s['image']+'.png','drawings','Stillwater Garden — '+s['image']+' — concept plan.png')
    ...
    for sid,title,description,section,position in [
        ('intro','Stillwater Garden. Step outside, feel elsewhere.',manifest['slides'][0]['body']+' All clients, suppliers and prices are fictional; imagery is AI-generated.','story',0),
        ('budget','The investment, made clear.','Fictional demo: €213,400–€221,400 including €18,000 contingency. Optional kitchen and rainwater storage begin unselected. Exceptional ground conditions remain unpriced. All amounts include assumed VAT.','budget',next(n for n,s in enumerate(manifest['slides']) if s['key']=='investment')),
        ...
        ('summary','Every detail, together.','Download the designed presentation, original images, 15 fictional supplier quotes, four design plans, material and planting schedules, programme and scope document.','delivery',len(manifest['slides'])+2),
    ]:
