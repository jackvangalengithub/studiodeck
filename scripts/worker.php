...
do {
    ...
    try {
        ...
        if($job['type']==='ingest') {
            ...
            $source=slide_image_source($slide);$path=tempnam(sys_get_temp_dir(),'sd-slide-');file_put_contents($path,$source['data']);
            try{$r=ai_request('images/edits',['model'=>env('OPENAI_IMAGE_MODEL','gpt-image-1'),'image'=>new CURLFile($path,$source['mime'],'source-image'),'prompt'=>'Edit this interior design image. Preserve camera position, geometry and all elements not mentioned. '.$payload['prompt'],'n'=>1,'size'=>'auto','output_format'=>'png'],true);}finally{unlink($path);}
            ...
            $payload=json_decode($job['payload'],true);$path=tempnam(sys_get_temp_dir(),'sd-edit-');file_put_contents($path,$v['data']);
            try{$r=ai_request('images/edits',['model'=>env('OPENAI_IMAGE_MODEL','gpt-image-1'),'image'=>new CURLFile($path,$v['mime'],$v['name']),'prompt'=>'Edit this interior design image. Preserve camera position, geometry and all elements not mentioned. '.$payload['prompt'],'n'=>1,'size'=>'auto','output_format'=>'png'],true);}finally{unlink($path);}
            $raw=base64_decode($r['data'][0]['b64_json']??'',true);if(!$raw||!@getimagesizefromstring($raw))throw new RuntimeException('The image service did not return a valid image.');
