...
do {
    ...
    try {
        ...
        if($job['type']==='ingest') {
            ...
            try{$r=ai_request('images/edits',['model'=>env('OPENAI_IMAGE_MODEL','gpt-image-1'),'image'=>new CURLFile($path,$source['mime'],'source-image'),'prompt'=>($payload['mode']==='photorealistic'?'Create an ultra-photorealistic architectural photograph from this render. It must look like a real photograph taken with a professional camera, with physically plausible lighting, camera optics, exposure, material microtexture, reflections and contact shadows. Avoid a synthetic CGI appearance, plastic surfaces, artificial glow and oversharpening. Preserve the exact design, geometry, furniture, finishes, colours, composition, aspect ratio and camera position. Add some daily small clutter. Keep these everyday items subtle and appropriate to the room; do not add or replace furniture or architectural elements. ':'Edit this interior design image. Preserve camera position, geometry and all elements not mentioned. ').$payload['prompt'],'n'=>1,'size'=>'auto','output_format'=>'png'],true);}finally{unlink($path);}
