// Render and validate the Haus Morgenlicht deck, plans and fictional quotes.
const {chromium}=require(process.env.PLAYWRIGHT_MODULE||'playwright');
const fs=require('node:fs'),path=require('node:path');
const {pathToFileURL}=require('node:url');
const root=path.resolve(__dirname,'../demos/haus-morgenlicht');
...
(async()=>{
 ...
 try{
  ...
  for(let i=0;i<slides.length;i++){
   ...
   const bad=await slides[i].evaluate(el=>[...el.querySelectorAll('h1,p,li,table,.total,.cards,.timeline-grid,.tree,.points,.diptych')].filter(x=>{const b=x.getBoundingClientRect(),p=el.getBoundingClientRect();return b.bottom>p.bottom-48||b.right>p.right+1;}).map(x=>x.tagName+': '+x.textContent.slice(0,70)));
