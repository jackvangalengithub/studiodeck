...
export function installGroupOrdering({getOrder,saveOrder,setBusy,onError}){
 ...
 function locate(){if(!drag?.active)return;const hit=document.elementFromPoint(drag.x,drag.y),row=hit?.closest('[data-group-id]');drag.valid=false;
  ...
  if(!row||row.dataset.groupId===drag.id){drag.valid=!!drag.target&&!!hit?.closest('.editor-group-list');return;}
 ...
 function scroll(){if(!drag?.active)return;const rail=drag.row.closest('.editor-group-list'),rect=rail.getBoundingClientRect();if(drag.y>=rect.top&&drag.y<=rect.bottom){const delta=drag.x<rect.left+35?-10:drag.x>rect.right-35?10:0;if(delta){rail.scrollLeft+=delta;locate();}}frame=requestAnimationFrame(scroll);}
 async function commit(order,id){saving=true;setBusy(true);try{await saveOrder(order);document.querySelector(`[data-drag-group="${CSS.escape(id)}"]`)?.focus({preventScroll:true});announce(tr("studio_group_order_saved"));}catch(e){onError(e.message);}finally{saving=false;setBusy(false);}}
