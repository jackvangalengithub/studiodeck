...
export function billingUi({state,api,esc,button,openModal,closeModal,render,applySession,resetStudio,toast,isModalOpen,resourceHeaders,resumeAccess=async()=>false,hasIntent=()=>false,purchaseStarted=()=>{},createProject=async()=>{}}){
  ...
  function page(){
    ...
    return `<div class="project-head"><h1>${tr("studio_billing_2")}</h1><div class="row billing-actions">${button(tr("studio_refresh"),'billing-refresh','','','history')}${current.has_customer?button(tr("studio_manage_payments"),'billing-portal','primary','','budget'):''}</div></div>
      ...
      }).join('')}</div></section></div><p class="form-hint">${tr("studio_prices_exclude_vat_extra_active_projects_10_month_each_extra_practice_designers_20_month_each_add_on")}</p>${!c.pass.available?`<p class="notice">${tr("studio_checkout_will_be_available_once_the_studio_s_payment_connection_is_configured")}</p>`:''}</section>
      ${addonsSection()}
  ...
  function addonState(a){
    ...
    if(['past_due','unpaid'].includes(a.status))return 'attention';
    if(a.active)return 'active';
    if(addonSubscribed(a))return 'pending';
    return 'inactive';
  }
  function addonsSection(){
    const entries=(current.addons||[]).filter(a=>addonDefinitions[a.id]);if(!entries.length)return '';
    return `<section class="billing-section billing-addons" aria-labelledby="billing-addons-title"><header class="billing-addons-heading"><div><p class="billing-eyebrow">${tr('billing_addons_eyebrow')}</p><h2 id="billing-addons-title">${tr('billing_addons_title')}</h2><p class="muted">${tr('billing_addons_description')}</p></div><span class="billing-optional-badge">${tr('billing_addons_optional')}</span></header><div class="billing-addon-grid">${entries.map(addonCard).join('')}</div></section>`;
  }
  function addonCard(a){
    const def=addonDefinitions[a.id],status=addonState(a),subscribed=addonSubscribed(a),pending=current.orders.some(o=>o.plan===a.id&&o.status==='pending');
    return `<article class="billing-addon-card" data-addon="${esc(a.id)}"><header class="billing-addon-heading"><div class="billing-addon-mark" aria-hidden="true">${esc(def.mark)}</div><div><h3>${tr(def.title)}</h3><span class="billing-addon-status is-${status}">${tr('billing_addon_status_'+status)}</span></div></header><p class="billing-addon-description">${tr(def.description)}</p><ul class="billing-addon-features">${def.features.map(key=>`<li>${tr(key)}</li>`).join('')}</ul><div class="billing-addon-footer"><div><p class="billing-price">${money(a.price,a.currency)}<small> ${esc(a.currency.toUpperCase())} ${tr('studio_month_2')}</small></p><p class="form-hint">${tr('billing_addon_price_note')}</p></div><div class="billing-addon-actions">${a.active?button(tr('billing_addon_open',{name:tr(def.title)}),def.open,'primary'):subscribed?button(tr('billing_addon_manage'),'billing-portal','primary'):button(tr(pending?'billing_addon_continue':'billing_addon_activate',{name:tr(def.title)}),'billing-addon-activate','primary',`data-addon="${esc(a.id)}" ${a.available?'':'disabled'}`)}${!a.active?button(tr('billing_addon_explore',{name:tr(def.title)}),def.open,'ghost'):subscribed&&!a.local?button(tr('billing_addon_manage'),'billing-portal','ghost'):''}</div></div>${a.local?`<p class="billing-addon-note">${tr('billing_addon_local_note')}</p>`:!a.available&&!a.active&&!subscribed?`<p class="billing-addon-note">${tr('billing_addon_unavailable_note')}</p>`:''}</article>`;
  }
  function banner(){
