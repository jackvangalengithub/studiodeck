...
export async function demoRequest(action,body={}) {
  if(['save_slide','project_settings','studio_theme'].includes(action)&&body instanceof FormData)body=Object.fromEntries(body);
  ...
  if(action==='studio_theme'){if(typeof body.theme==='string')body.theme=JSON.parse(body.theme);if(body.logo?.size){demoStudio.logo=await new Promise((resolve,reject)=>{const reader=new FileReader();reader.onload=()=>resolve(reader.result);reader.onerror=reject;reader.readAsDataURL(body.logo);});demoStudio.has_logo=true;}else if(body.remove_logo){delete demoStudio.logo;demoStudio.has_logo=false;}if(['interior','landscape','architecture','furniture','events'].includes(body.business_type))demoStudio.business_type=body.business_type;if(body.language){if(!['en','nl'].includes(body.language))throw Error('Choose English or Dutch.');demoStudio.language=body.language;}if(body.name)demoStudio.name=body.name;studioTheme=body.theme;return {studio_theme:studioTheme};}
