...
export function videoSlide(def){
 ...
 return `<section class="video-slide"><p class="slide-label">YouTube video</p><h1 class="slide-heading">${esc(def.title)}</h1>${def.record?.description?`<p class="slide-description">${esc(def.record.description)}</p>`:''}${video?`<div class="video-player"><button class="youtube-play" type="button" data-play-youtube="${video.id}" data-video-start="${video.start}" data-video-title="${esc(def.title)}" aria-label="Play video: ${esc(def.title)}">${playMark}<strong>Play video</strong><small>YouTube</small></button></div><a class="video-external-link" href="${esc(video.url)}" target="_blank" rel="noopener noreferrer">Watch on YouTube ↗</a>`:'<p class="notice">Add a YouTube link using Edit slide.</p>'}</section>`;
}
export function installVideoPlayers(){
 document.addEventListener('click',event=>{
  ...
  const frame=document.createElement('iframe');frame.dataset.youtubePlayer='';frame.title=button.dataset.videoTitle||'YouTube video';
