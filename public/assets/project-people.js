export function projectPeople({getData,api,openModal,closeModal,button,formFooter,esc,personAvatar,refresh,toast,addTeam}){
 ...
 async function submit(type,form){await api(type==='project-person'?'save_project_person':'remove_project_person',{...Object.fromEntries(new FormData(form)),project_id:getData().project.id});closeModal();await refresh(true);toast(type==='project-person'?'Contact details saved.':'Person removed from project.');}
