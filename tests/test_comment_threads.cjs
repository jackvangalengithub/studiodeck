...
(async()=>{
 ...
 try{
  ...
  await page.goto(slide);await page.locator('.presentation-sidebar-handle').hover();await page.locator('[data-action=feedback]').first().click();await page.locator('.comment-thread').first().waitFor();await page.getByRole('button',{name:'Sort: Newest first',exact:true}).click();
