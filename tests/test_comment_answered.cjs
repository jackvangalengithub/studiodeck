...
(async()=>{
 ...
 try{
  ...
  await page.goto(slide);await page.locator('.presentation-sidebar-handle').hover();await page.locator('[data-action=feedback]').first().click();await page.locator('.modal .comment-thread').first().waitFor();assert.deepEqual(await ids(),[c.id,a.id]);
  ...
  const shared=(await api('share',{iteration:iid,client_emails:['answer-client@example.test']})).links[0],client=await browser.newContext(),viewer=await client.newPage();viewer.on('pageerror',e=>errors.push(e.message));await viewer.goto(shared.url);await viewer.locator('.presentation-sidebar-handle').hover();await viewer.locator('[data-action=feedback]').first().click();await viewer.locator('.comment-thread').first().waitFor();assert.equal(await viewer.locator('.comment-thread').count(),1);
