...
(async()=>{
  ...
  try{
    fixture.setupCompleted=null;fixture.needsSetup=true;
    ...
    await page.locator('[data-studio-setup] [type=submit]').click();
    assert.equal(await page.locator('.setup-type').count(),6);
    ...
    assert.equal(fixture.needsSetup,false);assert.equal(await page.locator('[data-form=billing-onboard]').count(),0);await page.locator('.billing-trial-bar').getByText('5 days left in your trial').waitFor();
    ...
    await page.locator('[data-action=website]').click();assert.equal(await page.locator('[data-form=billing-onboard]').count(),0);await page.locator('.website-welcome').waitFor();
    ...
    await page.locator('[data-action=settings]').click();await page.locator('[name=business_type]').selectOption('signmaker');
    await page.locator('[data-form=studio-theme] [type=submit]').click();await page.locator('.modal').waitFor({state:'detached'});
    assert.equal(fixture.businessType,'signmaker');assert.match(await page.locator('.website-cover-frame img').getAttribute('src'),/signmaker.webp/);
    await page.locator('[data-action=projects]').first().click();await page.locator('.onboarding-welcome').waitFor();
    assert.match(await page.locator('.onboarding-film img').getAttribute('src'),/signmaker.webp/);
    // Every studio needs its own setup; a second studio starts at language again.
    fixture.studio='second';fixture.setupCompleted=null;fixture.needsSetup=true;fixture.language='nl';
    ...
    await page.locator('[value=signmaker]').focus();await page.keyboard.press('Space');assert.equal(await page.locator('[value=signmaker]').isChecked(),true);
    ...
    assert.equal(await page.locator('html').getAttribute('lang'),'nl');await page.locator('.billing-trial-bar').getByText('Nog 5 dagen in je proefperiode').waitFor();assert.equal(await page.locator('[data-form=billing-onboard]').count(),0);
    const bar=await page.locator('.billing-trial-bar').boundingBox();assert.equal(bar.x,0);assert.equal(bar.y,0);assert.equal(bar.width,await page.evaluate(()=>window.innerWidth));assert.equal(bar.height,44);
    await page.evaluate(()=>window.scrollTo(0,400));assert.equal((await page.locator('.billing-trial-bar').boundingBox()).y,0);await page.screenshot({path:artifacts+'/trial-bar-mobile-nl.png',fullPage:false});
    assert.match(await page.locator('.onboarding-film img').getAttribute('src'),/signmaker.webp/);
