...
2. Run `php scripts/setup-stripe.php`. It creates/reuses EUR prices for the €19 pass, €15 extension, €39/€199/€399 monthly plans, €10 extra projects and €20 extra team members, €39 Website, and a portal configuration. Copy the printed price/configuration IDs into the environment. This script does not print or write your secret key.
...
- New projects require an unused trial allocation, an available subscription slot, or an unused paid Project Pass in the same studio. Billing shows Project Pass first in its own bordered group, then the three monthly packages together in a separate bordered group. Optional modules appear inside each subscription package column. No unpaid project draft is created. Pending, failed or refunded orders provide no creation rights. Pass redemption and project creation share one transaction; rollback preserves the pass, and deleting a created project does not return it. Legacy exemptions preserve existing project access only, not permission to create new projects.
...
Subscription package columns include a Website toggle that adds **€39/month** to the displayed total and the actual Stripe subscription as one recurring line item. The selected option is sent with Checkout and package updates, and persists after verified payment. Website access follows the paid Website item: pending payment does not unlock publishing, and removing the item ends Website access. Legacy separate Website subscriptions remain supported and cannot be added again to a package while still active. Website does not consume seats or project slots.
