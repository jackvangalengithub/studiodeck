...
2. Run `php scripts/setup-stripe.php`. It creates/reuses EUR prices for the €19 pass, €15 extension, €39/€199/€399 monthly plans, €10 extra projects and €20 extra Practice designers, and a portal configuration. Copy the printed price/configuration IDs into the environment. This script does not print or write your secret key.
...
Portal plan changes must remain disabled. The app validates seats/project capacity and previews proration before changing packages; reductions are scheduled for renewal. The portal handles payment methods, invoice history, billing details and cancellation at period end. A pending change is displayed in Billing; retrying confirmation uses the same Stripe idempotency key.

## Signup and coverage

- Email-link sign-in also registers a new studio. Verification is followed by a studio/name setup form; only submitting verified onboarding starts the 7-day trial. Invitations do not create trials.
...
- Project cards, creation, direct project/slide URLs and legacy preview URLs use the access dialog. Private read-only access remains available. Members see an admin contact message instead of purchase controls; clients receive neutral unavailable messages.
