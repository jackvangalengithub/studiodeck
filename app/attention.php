...
function studio_attention(array $u,string $kind='all',int $offset=0): array {
    ...
    candidates AS (
        SELECT 'questions' AS kind,q.id,p.id AS project_id,p.name AS project_name,i.id AS iteration_id,i.number AS iteration_number,
            q.question AS title,q.created_at AS occurred_at,'' AS deadline,'' AS recipient_name,0 AS assigned_to_me,0 AS unread_count,3 AS priority,COALESCE((SELECT root_id FROM checklist_threads qt WHERE qt.iteration_id=q.iteration_id AND qt.question_id=q.id),'question:'||q.id) AS subject_id
        FROM open_questions q JOIN iterations i ON i.id=q.iteration_id JOIN scope p ON p.id=i.project_id
        WHERE (q.accepted=1 OR q.published=1) AND q.dismissed=0 AND q.resolved=0
            AND NOT EXISTS (SELECT 1 FROM comment_confirmations linked WHERE linked.comment_id=q.confirmation_id AND linked.status='pending')
        ...
        SELECT 'confirmations',c.id,p.id,p.name,i.id,i.number,c.body,c.created_at,'',r.recipient_name,r.recipient=?,0,
            CASE WHEN r.recipient=? THEN 1 ELSE 2 END,COALESCE(c.parent_id,c.id)
        ...
        UNION ALL
        SELECT 'feedback',root.id,p.id,p.name,i.id,i.number,root.body,MIN(c.created_at),'','',0,COUNT(*),4,root.id
        ...
        SELECT 'deadlines',p.id,p.id,p.name,i.id,i.number,p.name,d.deadline,d.deadline,'',0,0,
            CASE WHEN d.deadline<? THEN 0 ELSE 5 END,'deadline:'||p.id
    ...
    ), ranked AS (SELECT *,ROW_NUMBER() OVER (PARTITION BY project_id,subject_id ORDER BY priority,occurred_at,id) AS subject_rank FROM candidates),
    attention AS (SELECT * FROM ranked WHERE subject_rank=1) ";
