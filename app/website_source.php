...
function website_templates(?string $type=null): array {
    ...
    $styles=[
        'editorial'=>['editorial','classic'], 'linen'=>['classic','organic'],
        'noir'=>['modern','editorial'], 'gallery'=>['modern','minimal'],
        'coast'=>['organic','minimal'], 'atelier'=>['classic','editorial'],
        'panorama'=>['modern','editorial'], 'folio'=>['modern','minimal'],
        'terracotta'=>['organic','classic'], 'minimal'=>['modern','minimal'],
    ];
    foreach($templates as &$template){
        $template['styles']=$styles[$template['id']];
        $template['business_types']=array_values(array_column(array_filter(studio_business_types(),fn($business)=>in_array($template['id'],$business['templates'],true)),'id'));
        if(!$template['business_types'])$template['business_types']=array_column(studio_business_types(),'id');
    }
    unset($template);
    // Three expressive directions for every discipline, with distinct palettes and layouts.
    $collections=[
        'interior'=>[['Scrapbook','Layered photographs, paper edges and a playful studio journal.'],['Color Field','Cobalt blue, oversized type and confident blocks of color.'],['Soft Sculpture','Circular photographs and a lilac, sculptural composition.']],
        'landscape'=>[['Field Notes','A botanical scrapbook with offset photographs and earthy paper.'],['Wild Type','Acid yellow, bold lettering and landscapes at poster scale.'],['Earth Orbit','Terracotta circles and generous, flowing garden photography.']],
    ...
    foreach($collections as $business=>$designs)foreach($designs as $i=>$design){
        ...
        $templates[]=['id'=>$business.'-'.$layout,'name'=>$design[0],'description'=>$design[1],'tone'=>'expressive','business_types'=>[$business],'styles'=>['experimental',['editorial','modern','organic'][$i]],'layout'=>$layout];
    }
    $templates[]=['id'=>'blank','name'=>'Blank page','description'=>'An open canvas. Build your own direction from the first word.','tone'=>'light','business_types'=>array_column(studio_business_types(),'id'),'styles'=>[]];
    if($type!==null){
        $recommended=studio_business_profile($type)['templates'];
        foreach($templates as &$template)$template['recommended']=$template['id']!=='blank'&&in_array($template['id'],studio_business_profile($type)['templates'],true);
        unset($template);
        usort($templates,fn($a,$b)=>(array_search($a['id'],$recommended,true)===false?($a['id']==='blank'?100:99):array_search($a['id'],$recommended,true))<=>(array_search($b['id'],$recommended,true)===false?($b['id']==='blank'?100:99):array_search($b['id'],$recommended,true)));
    }
    return $templates;
}
...
function website_seed_files(array $d,string $template,?string $hero=null): array {
    ...
    if($template==='blank')return [
        'index.html'=>'<!doctype html><html lang="'.($d['language']??'en').'"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>'.$e($d['title']).'</title><meta name="description" content="'.$e($d['description']).'"><link rel="stylesheet" href="styles.css"><script src="script.js" defer></script></head><body><main id="main"><h1>'.$name.'</h1></main></body></html>',
        'styles.css'=>'* { box-sizing: border-box; } body { margin: 0; padding: 8vw; background: #faf9f6; color: #353330; font-family: system-ui, sans-serif; } h1 { overflow-wrap: anywhere; } img { max-width: 100%; height: auto; }',
        'script.js'=>'',
    ];
    $definition=array_column(website_templates(),null,'id')[$template];
    $bodyClass=$template.' '.($definition['layout']??'');
...
/* Expressive starting designs: collage, graphic poster and circular gallery. */
.collage{--paper:#eee4d5;--ink:#47342c;--muted:#705b50;--line:#cdbba6;--accent:#b8442e}.collage .hero{grid-template-columns:1.1fr 1fr;gap:4%;padding:100px 3%}.collage h1{font-style:italic;font-size:clamp(52px,7.8vw,120px);position:relative;z-index:1}.collage .hero-image{padding:18px 18px 52px;background:#fffaf2;transform:rotate(5deg);box-shadow:14px 18px 0 var(--line)}.collage .hero-image img{aspect-ratio:1/1}.collage .image-caption{font-family:monospace}.collage .work-grid{gap:70px;padding:20px}.collage .work-card{background:#fffaf2;padding:18px;box-shadow:8px 10px 0 var(--line);transform:rotate(-3deg)}.collage .work-card:nth-child(even){transform:rotate(3deg);margin-top:90px}.collage .studio{border-top:2px dashed var(--ink)}.collage .contact h2{font-style:italic}.collage .contact{margin-top:40px;background:var(--accent);color:var(--paper);padding:80px 20px}
.poster{--paper:#2345c8;--ink:#fff2bc;--muted:#e0e5ff;--line:#8194e0;--accent:#fff2bc}.poster h1,.poster h2,.poster h3{font-family:Arial,sans-serif;font-weight:900;text-transform:uppercase;letter-spacing:-.065em}.poster .site-header{border-bottom:3px solid var(--ink)}.poster .hero{display:flex;flex-direction:column;align-items:stretch;gap:40px;padding:50px 0}.poster h1{font-size:clamp(60px,11vw,170px);max-width:1200px;line-height:.9}.poster .hero-copy{display:grid;grid-template-columns:minmax(0,2fr) minmax(0,1fr);gap:25px;align-items:end}.poster .hero-copy h1{overflow-wrap:anywhere;grid-column:1/-1;grid-row:2}.poster .hero-copy .eyebrow{grid-column:1/-1}.poster .intro{margin:0}.poster .text-link{justify-self:end}.poster .hero-image img{aspect-ratio:2.5/1;filter:saturate(.8)}.poster .image-caption{display:none}.poster .work-card{border-top:6px solid var(--ink);padding-top:20px}.poster .work-card h3{font-size:36px}.poster .contact{background:var(--ink);color:var(--paper);padding:90px 5%}.poster .site-footer{margin-top:40px}
.orbit{--paper:#e9e1f4;--ink:#432e66;--muted:#66547e;--line:#beabd5;--accent:#7048a7}.orbit .hero{grid-template-columns:1.1fr 1fr;gap:3%;position:relative}.orbit h1{font-size:clamp(50px,7vw,110px)}.orbit .hero-image{padding:20px;border:1px solid var(--accent);border-radius:50%}.orbit .hero-image img{aspect-ratio:1/1;border-radius:50%}.orbit .image-caption{text-align:center;position:absolute;bottom:8%;left:15%;right:15%;border-radius:30px;background:var(--paper);padding:10px}.orbit .work-grid{gap:70px}.orbit .work-card img{border-radius:50% 50% 5px 5px;aspect-ratio:1/1}.orbit .work-card:nth-child(even){padding-top:110px}.orbit .studio{border-top:0;background:var(--ink);color:var(--paper);border-radius:180px 180px 8px 8px;padding:110px 8%;margin:40px 0}.orbit .contact h2{font-style:italic}.orbit .contact{border-top:0}.orbit blockquote{border-radius:30px;background:var(--line);padding:32px}
.landscape-collage{--paper:#ede9cf;--ink:#3f422e;--muted:#63664a;--line:#c9c7a4;--accent:#646c35}.architecture-collage{--paper:#dedbd5;--ink:#302f2d;--muted:#625e57;--line:#b8b2a8;--accent:#984830}.furniture-collage{--paper:#efdbbb;--ink:#533724;--muted:#796048;--line:#d1b288;--accent:#985d2d}.events-collage{--paper:#f9d0dd;--ink:#6c1740;--muted:#8b3b60;--line:#e4a3bb;--accent:#a72e5d}
.landscape-poster{--paper:#e8ef53;--ink:#26382a;--muted:#425337;--line:#a2b243;--accent:#26382a}.architecture-poster{--paper:#1735a9;--ink:#f4f2e8;--muted:#d4dcfa;--line:#899ae0;background-image:linear-gradient(#ffffff0d 1px,transparent 1px),linear-gradient(90deg,#ffffff0d 1px,transparent 1px);background-size:40px 40px}.furniture-poster{--paper:#f26932;--ink:#291d1b;--muted:#4d2f26;--line:#b5411d;--accent:#291d1b}.events-poster{--paper:#501bb1;--ink:#ecff70;--muted:#e2d3ff;--line:#9874d6;--accent:#ecff70}
.landscape-orbit{--paper:#f0d5be;--ink:#70402e;--muted:#815940;--line:#d7af8f;--accent:#995635}.architecture-orbit{--paper:#eae3dc;--ink:#622638;--muted:#785861;--line:#cfb1b8;--accent:#622638}.furniture-orbit{--paper:#f4eacc;--ink:#254baa;--muted:#526586;--line:#c5cde0;--accent:#c8452b}.events-orbit{--paper:#21142f;--ink:#f2c2ff;--muted:#c4a0d0;--line:#573263;--accent:#d6ed76}
@media(max-width:700px){.collage .hero,.orbit .hero{grid-template-columns:1fr;padding:45px 0;gap:40px}.collage .hero-image{transform:none;box-shadow:6px 8px 0 var(--line)}.collage .work-grid{padding:10px}.collage .work-card,.collage .work-card:nth-child(even){transform:none;margin-top:0}.poster .hero-copy{grid-template-columns:minmax(0,1fr)}.poster .text-link{justify-self:start;margin-top:0}.poster .hero-image img{aspect-ratio:4/3}.orbit .work-card:nth-child(even){padding-top:0}.orbit .studio{border-radius:90px 90px 8px 8px;padding:70px 8%}.orbit .hero-image{max-width:450px;width:100%;margin:auto}}
