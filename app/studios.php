...
function session_details(?array $s): array {
    ...
    return ['user'=>['feedback_reviewer'=>product_feedback_reviewer($s),'id'=>$s['user_id'],'email'=>$s['email'],'name'=>$s['name'],'profile'=>profile_for(person_key($s['email'],true),$s['name'])],'unread_count'=>unread_comment_count($s),'csrf'=>$s['csrf'],'studio'=>$studio,'studios'=>$studios,'studio_theme'=>clean_studio_theme(json_decode($studio['theme']??'{}',true)),'capabilities'=>capabilities(),'billing'=>$studio?billing_summary($studio['id']):null];
}
...
function studio_admin(array $u): void {if(!one("SELECT 1 FROM studio_members WHERE studio_id=? AND user_id=? AND role='admin'",[$u['studio_id'],$u['user_id']]))fail('Only studio admins can manage studio settings and billing.',403);}
