...
function billing_change_preview(array $u,array $input,bool $immediate=false): array {
    ...
    $catalog=billing_catalog();$desired=[$plan=>1];if($ep)$desired['extra_project']=$ep;if($es)$desired['extra_seat']=$es;
    ...
    foreach($desired as $key=>$qty){
        ...
        $existing=null;foreach($old as $item){if($item['price']===$price||(in_array($key,['solo','studio','practice'],true)&&billing_subscription_plan_for_price($item['price'],$u['studio_id'])!==null))$existing=$item;}
    ...
    $current=billing_studio($u['studio_id']);$oldCost=$catalog[$current['plan']]['cents']+1000*(int)$current['extra_projects']+2000*(int)$current['extra_seats'];$newCost=$catalog[$plan]['cents']+1000*$ep+2000*$es;
    ...
    if($plan===$current['plan']&&$ep===(int)$current['extra_projects']&&$es===(int)$current['extra_seats'])fail('This is already your current package.');
    ...
    $id=id();insert('billing_changes',['id'=>$id,'studio_id'=>$u['studio_id'],'subscription_id'=>$s['id'],'plan'=>$plan,'extra_projects'=>$ep,'extra_seats'=>$es,'parameters'=>json_encode(['items'=>$items,'schedule_items'=>$scheduleItems,'decrease'=>$decrease]),'snapshot'=>json_encode($old),'amount'=>$amount,'currency'=>$currency,'proration_at'=>$at,'created_at'=>$at,'effective_at'=>$effective]);
...
function billing_change_checkout(array $u,array $input): array {
    ...
    if($pending){
        if($pending['plan']!==$plan||(int)$pending['extra_projects']!==$ep||(int)$pending['extra_seats']!==$es)fail('Finish your existing package payment first.',409);
        $id=$pending['id'];
    }else{
