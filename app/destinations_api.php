<?php
if($action==='destinations')json_response(account_destinations(authenticated_user()));
if($action==='client_project'){
    $user=authenticated_user();$project=text_field($_GET['project_id']??'');if(!$project)fail('Choose a shared project.');$share=account_client_share($user,$project,text_field($_GET['iteration']??''));
