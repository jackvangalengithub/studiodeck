...
function plan_document_pages(array &$inventory,string $directory,?callable $request=null,?callable $parallelRequest=null): array {
    if(!$request&&!$parallelRequest&&env('OPENAI_API_KEY')==='')return [];
    require_once __DIR__.'/ai_transport.php';
    $parallelRequest??='ai_json_parallel';
    $plans=[];$factories=[];$started=hrtime(true);
    $concurrency=$request?1:document_page_concurrency();
    ...
    foreach($inventory['pages'] as $page) {
        ...
        $factories[$page['number']]=function()use($page,$directory,$prompt){
            $preview=@file_get_contents($directory.'/'.basename($page['preview']));
            if($preview===false)throw new RuntimeException('Page preview could not be read.');
            $content=[['type'=>'text','text'=>'PAGE '.$page['number']."\nText:\n".substr($page['text'],0,12000)."\nNative image geometry:\n".json_encode($page['candidates']??[],JSON_INVALID_UTF8_SUBSTITUTE)],
                      ['type'=>'image_url','image_url'=>['url'=>'data:image/jpeg;base64,'.base64_encode($preview),'detail'=>'high']]];
            return [$prompt,$content];
        };
    }
    $progress=fn($completed,$total,$active)=>processing_progress('classifying_pages',['completed'=>$completed,'total'=>$total,'active'=>$active]);
    $results=[];
    if($request){
        // Retain the single-request injection used by existing callers and tests.
        foreach($factories as $number=>$factory){
            $progress(count($results),count($factories),1);
            try{$results[$number]=['data'=>$request(...$factory())];}
            catch(Throwable $error){$results[$number]=['error'=>$error->getMessage()];}
        }
    }else{
        try{$results=$parallelRequest($factories,$concurrency,$progress);}
        catch(Throwable $error){/* A pool failure keeps every page's conservative extraction fallback. */}
    }
    $failures=0;
    foreach($factories as $number=>$factory){
        $result=$results[$number]['data']??null;
        if(!is_array($result)||($result['number']??null)!==$number||!in_array($result['content_type']??'',['photo','render','collage','moodboard','mixed','drawing','text','budget','cover','unknown'],true)||!is_array($result['regions']??null)){
            $inventory['warnings'][]='Page '.$number.': visual extraction planning unavailable. Preserve the composition and review its labels.';
            $failures++;continue;
        }
        $plans[$number]=$result;
    }
    $progress(count($factories),count($factories),0);
    error_log(json_encode(['event'=>'document_page_analysis','elapsed_ms'=>round((hrtime(true)-$started)/1e6),'pages'=>count($factories),'concurrency'=>$concurrency,'failures'=>$failures]));
    return $plans;
}
