...
function post_communication(array $b): array {
    return transaction(function()use($b){
        ...
        $parentId=text_field($b['parent_id']??'',80);if($scoped){if(($parentId!==''&&$parentId!==$scoped)||array_key_exists('thread_title',$b))fail('Reply within the invited conversation.',403);$parentId=$scoped;}$parent=$parentId?one('SELECT * FROM comments WHERE id=? AND iteration_id=?',[$parentId,$i['id']]):null;
        ...
        $c=['id'=>id(),'iteration_id'=>$i['id'],'parent_id'=>$parentId?:null,'slide'=>$parent['slide']??'general','author'=>$actor,'body'=>$body,'created_at'=>now()];insert('comments',$c);
        ...
        if($person)insert('comment_confirmations',['comment_id'=>$c['id'],'recipient'=>$recipient,'recipient_name'=>$person['name'],'amount_cents'=>$amount]);
