...
function queue_comment_notifications(array $i,array $c): void {
    ...
    foreach(rows('SELECT * FROM shares WHERE iteration_id=? AND revoked=0 AND expires_at>? ORDER BY created_at',[$i['id'],time()]) as $s){if($s['email']===$c['author']||isset($recipients[$s['email']]))continue;$key=person_key($s['email'],(bool)one('SELECT 1 FROM users WHERE email=?',[$s['email']]));if(!profile_for($key)['email_comments'])continue;$t=token();insert('share_aliases',['token_hash'=>hash_token($t),'share_id'=>$s['id']]);$recipients[$s['email']]=['key'=>$key,'user'=>null,'share'=>$s['id'],'url'=>base_url().'/?slide='.rawurlencode($c['slide']).'#/view/'.$t];}
