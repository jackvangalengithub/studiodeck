...
function queue_comment_notifications(array $i,array $c): void {
    ...
    foreach(rows('SELECT s.* FROM shares s JOIN iterations i ON i.id=s.iteration_id JOIN project_client_members cm ON cm.project_id=i.project_id AND cm.email=s.email WHERE s.iteration_id=? AND s.revoked=0 AND s.expires_at>? ORDER BY s.created_at',[$i['id'],time()]) as $s){if($s['email']===$c['author']||isset($recipients[$s['email']]))continue;$key=person_key($s['email'],(bool)one('SELECT 1 FROM users WHERE email=?',[$s['email']]));if(!profile_for($key)['email_comments'])continue;$recipients[$s['email']]=['key'=>$key,'user'=>null,'share'=>$s['id'],'url'=>base_url().'/client/projects/'.$i['project_id'].'?iteration='.$i['id'].'&slide='.rawurlencode($c['slide'])];}
...
function dispatch_comment_email(): bool {
    ...
    try{
        ...
        $sent=send_branded_email($job['email'],$c['project_id'],empty($c['parent_id'])?'New comment on your design presentation':'New reply on your design presentation',$c['author']." wrote:\n\n".$c['body'],$url,'Read comment');
