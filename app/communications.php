...
function email_template(string $pid,string $heading,string $message,string $url,string $label='Open presentation'): string {
    $p=one('SELECT p.name,s.name AS studio_name,s.theme FROM projects p JOIN studios s ON s.id=p.studio_id WHERE p.id=?',[$pid]);$t=json_decode($p['theme'],true)?:[];
    $palettes=['sage'=>['#465441','#f7f7f3'],'clay'=>['#794f40','#faf6f0'],'slate'=>['#3d566c','#f5f7fa'],'ink'=>['#303239','#f6f6f7'],'ocean'=>['#285b6b','#f3f8f8'],'plum'=>['#68445f','#faf6f9'],'rust'=>['#86482f','#fbf6f0'],'forest'=>['#30564a','#f5f8f3'],'mustard'=>['#776022','#faf9f1'],'rose'=>['#80515b','#fbf6f7'],'lavender'=>['#57527d','#f7f6fb'],'espresso'=>['#58483a','#faf7f2']];
    [$accent,$bg]=$palettes[$t['palette']??'sage']??$palettes['sage'];$h=fn($s)=>htmlspecialchars($s,ENT_QUOTES|ENT_SUBSTITUTE,'UTF-8');
    return '<!doctype html><html><body style="margin:0;background:'.$bg.';font-family:Arial,sans-serif;color:#292d2a"><table role="presentation" width="100%"><tr><td style="padding:36px 16px"><table role="presentation" style="max-width:600px;margin:auto;width:100%;background:#fff;border-top:6px solid '.$accent.'"><tr><td style="padding:32px"><p style="color:'.$accent.'">'.$h($p['studio_name']).'</p><h1>'.$h($heading).'</h1><h2 style="font-size:18px">'.$h($p['name']).'</h2><p style="line-height:1.7">'.nl2br($h($message)).'</p><p style="margin:30px 0"><a href="'.$h($url).'" style="display:inline-block;padding:14px 22px;background:'.$accent.';color:#fff;border-radius:6px;text-decoration:none">'.$h($label).'</a></p><p style="font-size:12px;color:#636b65">Manage comment emails in your profile inside Studiodeck or the client presentation.</p><p style="font-size:12px;color:#636b65">Presented by studiodeck</p></td></tr></table></td></tr></table></body></html>';
}
