...
function billing_require_fit(string $sid,string $plan,int $extraProjects=0,int $extraSeats=0,bool $includeTrial=false): void {
    ...
    $p=billing_catalog()[$plan];
    if($plan==='solo'&&$extraSeats>0)fail('Solo includes exactly one person.');
    if($plan==='studio'&&$p['seats']+$extraSeats>14)fail('Studio supports a maximum of 14 people. Choose Practice for a larger team.');
    $usage=billing_usage($sid);
...
function billing_sync_subscription(array $s): void {
    ...
    foreach($s['items']['data']??[] as $item){
        ...
        $key??=billing_subscription_plan_for_price($price,$b['studio_id']);
        ...
        elseif(env('STRIPE_PRICE_WEBSITE')&&$price===env('STRIPE_PRICE_WEBSITE')&&(int)$item['quantity']===1)continue; // Historical add-on; never sold by new checkouts.
    ...
    website_sync_package($s,$b['studio_id']);
