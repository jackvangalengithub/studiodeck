...
function deck_payload(array $i, bool $isOwner): array {
    ...
    if($isOwner) {
        ...
        foreach($result['jobs'] as &$job) { $job['progress']=(json_decode($job['payload'],true)?:[])['progress']??null;unset($job['payload']); }unset($job);
