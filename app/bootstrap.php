...
function authenticated_user(bool $write=false): array {
...
function owner(bool $write=false): array {
    $s=authenticated_user($write);
    require_studio_member($s);
    return $s;
}
function require_studio_member(array $s): void {
    if(!one('SELECT 1 FROM studio_members WHERE studio_id=? AND user_id=?',[$s['studio_id'],$s['user_id']]))fail('Studio membership is required.',403);
}
function owned_project(string $pid,array $u,bool $write=true): array {
    require_studio_member($u);
    ...
    if($write)billing_require_project($pid,$u['user_id']);
    return $p;
}
function owned_iteration(string $iid,array $u,bool $editable=false,bool $write=false): array {
    ...
    if($editable&&!empty($i['locked']))fail('This iteration is locked. Ask a studio admin to unlock it, or create a new iteration to make changes.',409);
    return $i;
}
function access_iteration(string $iid='', bool $write=false): array {
    $auth=$_SERVER['HTTP_AUTHORIZATION']??'';
    if(str_starts_with($auth,'Client ')) {
        $user=authenticated_user($write);$share=account_client_share($user,'',$iid,substr($auth,7));billing_require_project($share['project_id'],null,true,$write);
        return [one('SELECT * FROM iterations WHERE id=?',[$share['iteration_id']]),$user['email'],false,$share];
    }
    if($auth!=='' && !str_starts_with($auth,'Client '))fail('Sign in to access this presentation.',401);
...
function project_files(string $iid): array {
    ...
    foreach($files as &$f) {
        ...
        $reference=one('SELECT v.title,v.revision,m.asset_id,m.slide_id,m.fingerprint FROM iteration_pack_items m JOIN studio_pack_versions v ON v.id=m.version_id JOIN studio_pack_items t ON t.id=m.item_id WHERE m.iteration_id=? AND m.asset_id=? AND m.excluded=0 AND t.kind="document"',[$iid,$f['asset_id']]);
        $f['studio_reference']=$reference?['title'=>$reference['title'],'revision'=>$reference['revision'],'modified'=>$reference['fingerprint']!==pack_fingerprint($iid,$reference)]:null;
        require_once __DIR__.'/documents.php';
        $f['pages']=document_page_summaries($f['id']);
        $f['history']=[]; $vid=$f['id'];
        while($vid) { $v=one('SELECT id,parent_id,number,name,mime,size,metadata,created_at FROM file_versions WHERE id=?',[$vid]); if(!$v)break; $meta=json_decode($v['metadata'],true)?:[];if(!empty($meta['generated']))$v['enhancement_summary']=enhancement_prompt_summary($meta['prompt']??'');unset($v['metadata']);$f['history'][]=$v; $vid=$v['parent_id']; }
    }
    return $files;
}
function deck_payload(array $i, bool $isOwner): array {
    ...
    $p=array_merge($p,project_language($p['id']));
    $p['theme']=json_decode($i['theme'],true)?:json_decode($p['theme'],true)?:[];
    $items=budget_rows($i['id']);
    $files=project_files($i['id']);
    if(!$isOwner){foreach($files as &$file)unset($file['metadata']['google_drive']);unset($file);}
